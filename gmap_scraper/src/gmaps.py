...
def print_social_errors(credits_exhausted, not_subscribed, unknown_error):
    ...
    if credits_exhausted:
        print(f"""Could not get social details for {
            len(credits_exhausted)} places due to credit exhaustion. Please consider upgrading your plan by visiting https://rapidapi.com/Chetan11dev/api/website-social-scraper-api/pricing to continue scraping social details.""")

    if not_subscribed:
        print(f"""Could not get social details for {len(
            not_subscribed)} places as you are not subscribed to Website Social Scraper. Please subscribe to a free plan by visiting https://rapidapi.com/Chetan11dev/api/website-social-scraper-api/pricing to scrape social details.""")

    if unknown_error:
        print(f"""Could not get social details for {
              len(unknown_error)} places due to Unknown Error.""")
...
def merge_social(places, social_details):
    for place in places:
        found_social_detail = next(
            (detail for detail in social_details if detail['place_id'] == place['place_id']), None)
...
def merge_reviews(places, reviews):
    for place in places:
        ...
        found_review = next(
            (review for review in reviews if review['place_id'] == place['place_id']), None)
...
def process_result(min_reviews, max_reviews, category_in, has_website, has_phone, min_rating, max_rating, sort, key, scrape_reviews, reviews_max, reviews_sort, fields, lang, should_scrape_socials, convert_to_english, cache, places_obj):
    ...
    # 2. Scrape Emails
    if should_scrape_socials:
        places_with_websites = filter_places(
            cleaned_places, {"has_website": True})
        social_data = create_social_scrape_data(places_with_websites, key)
        social_details = bt.remove_nones(
            scrape_social(social_data, cache=cache))
        success, credits_exhausted, not_subscribed, unknown_error = clean_social(
            social_details)
    ...
    if scrape_reviews:
        ...
        reviews_data = create_reviews_data(
            placed_with_reviews, reviews_max, reviews_sort, convert_to_english, lang)
...
class Gmaps:
    ...
    DEFAULT_SORT = [SORT_BY_REVIEWS_DESCENDING, SORT_BY_HAS_WEBSITE,
                    SORT_BY_NOT_HAS_LINKEDIN, SORT_BY_IS_SPENDING_ON_ADS]
    ALL_REVIEWS = None
    ...
    def links(
        ...
        places = scraper.scrape_places_by_links(
            {"links": links, "convert_to_english": convert_to_english, "cache": use_cache}, cache=use_cache)
