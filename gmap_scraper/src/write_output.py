...
def transform_about(about_list):
    ...
    for item in about_list:
        ...
        transformed_about[enabled_key] = ", ".join(
            enabled_options) if enabled_options else ""
        transformed_about[disabled_key] = ", ".join(
            disabled_options) if disabled_options else ""
    ...
    if isinstance(data, dict):
        ...
        for day, times in data.items():
            ...
            for time_slot in times:
                ...
                popularity_percentage = time_slot.get(
                ...
                formatted_output += f"    {time_label}: {
                    popularity_percentage}% | {popularity_description}\n"
...
def most_popular_times_to_string(data):
    ...
    if isinstance(data, list):
        ...
        xs = []
        for el in data:
            average_popularity = el.get(
                "average_popularity", "No Average Popularity")
    ...
    for place in places:
        ...
        for field in fields:
            if field == Fields.REVIEWS_PER_RATING:
                ...
                transformed_place[Fields.FEATURED_QUESTION] = featured_question_to_string(
                    place[Fields.FEATURED_QUESTION])
                ...
                coords = f"{place[Fields.COORDINATES]['latitude']},{
                    place[Fields.COORDINATES]['longitude']}"
                ...
                # Formatting hours
                hours = '\n'.join(
                    [f"{day['day']}: {', '.join(day['times'])}" for day in place['hours']])
...
def create(places, selected_fields, csv_path, json_path, query_kebab):
    ...
    if can_create_detailed_reviews_csv(selected_fields):
        detailed_reviews_path = csv_path + \
            format(query_kebab, "csv", "detailed-reviews")
        # "detailed-reviews.csv"
        written.append(detailed_reviews_path)
        create_detailed_reviews_csv(
            detailed_reviews_path, places, selected_fields)
