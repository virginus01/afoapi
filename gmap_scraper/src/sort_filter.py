...
def sort_place(places: list, sort):
    ...
    def sorting_key(item):
        ...
        if value is None:
            # A tuple with a single element to ensure type consistency
            return (0,)
...
def list_contains_string(string_list, target_string):
    target_string_lower = kebab_case(unicode_to_ascii(
        target_string)).lower()  # Convert target string to lowercase
    for item in string_list:
        # Compare in a case-insensitive manner
        if target_string_lower == kebab_case(unicode_to_ascii(item)).lower():
            return True
    return False
...
def sort_dict_by_keys(dictionary, keys):
    ...
    for key in keys:
        if key in dictionary:
            ...
            # Handle the case where a key is not present in the original dictionary
